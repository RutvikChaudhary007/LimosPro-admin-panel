...
function AddRegionAdmin() {
  const { data: regionData, isFetching: regionFetching } = useFetchAllRegions({ limit: 100 })
      ...
      <Header className="mt-4 mb-5 h-[79px] rounded-[6px] bg-[#FDFDFD] p-4 shadow-[0_4px_20px_rgba(0,0,0,0.05)]">
        <div className="">
          ...
          <h4>
            ...
            <span className="h-4 w-[50px] text-xs text-[#3A3A3A]">/ Create Region Admin</span>
          </h4>
        </div>
      </Header>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="col-span-2 col-start-1 mb-[31px] flex flex-col gap-3">
                  <FormLabel className="text-sm text-black">Region</FormLabel>
                  {regionFetching ? (
                    <Spinner />
                  ) : (
                    <Select onValueChange={field.onChange}>
                      ...
                      <SelectContent className="col-span-2 col-start-1">
                        {regionData &&
                          regionData?.regions?.map((region: { regionName: string; id: string }) => (
