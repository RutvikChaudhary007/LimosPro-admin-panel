...
const AddressInput = ({ 
    ...
    if (isLoaded && addressInputRef.current && !loadError) {
      ...
      if (!isValid) {
        Toaster({
          ...
          description: "Please provide a full address with street number, route name, city, state, zip, and country.",
        });
  ...
  const handleManualFieldChange = (field: keyof AddressFields, value: string) => {
