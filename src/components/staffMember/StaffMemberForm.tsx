...
//   .refine((pw) => /[!@#$%^&*]/.test(pw), { message: "Needs a special character" }),
    role: z.enum(["admin", "chauffeur", "affiliate"]).array().min(1).or(z.enum(["admin", "chauffeur", "affiliate"])),
});
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="w-full col-span-full">
                                    <FormLabel className="placeholder-[#E6E6E6] font-medium">Select Permission</FormLabel>
                                    <Select value={field.value[0]}  onValueChange={(v) => {
                                        field.onChange(v);
                                        ...
                                        <SelectContent className="">
                                            {["admin", "chauffeur", "affiliate"].map((option,id) => (
                                                <SelectItem className="cursor-pointer" key={id} value={option}>{option}</SelectItem>
                                            ))}
                                        </SelectContent>
