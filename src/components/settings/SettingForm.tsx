...
const formSchema = z.object({
    paymentId: z.string().refine(value => value.trim() !== "", {
    ...
    phone: z
            ...
            .refine((n) => n >= 0, { message: "Must be non‑negative" }).nullable(),
    whatsapp: z
            .string()
    ...
    skype: z
            .string()
            .min(1, { message: "skype number or Id is required" }).optional(),
            // .regex(/^\d+$/, { message: "Must be number" })
            // .transform((v) => Number(v))
            // .refine((n) => n >= 0, { message: "Must be non‑negative" }),
    paymentSecretKey: z.string().refine(value => value.trim() !== "", {
                         ...
                         <FormField
                             ...
                             render={({ field }) => (
                                 <FormItem className="col-span-2 ">
                                     ...
                                     <FormMessage
                                         className={`mt-1 h-5 ${form.formState.errors.paymentSecretKey ? 'visible text-red-600' : 'invisible'
                                             }`}
                                     >
