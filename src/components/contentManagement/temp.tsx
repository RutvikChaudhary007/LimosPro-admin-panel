  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <Controller
        ...
        render={({ field }) => <input {...field} placeholder="Field Label" className="border p-2 rounded" />}
      />
      ...
      >
        <SortableContext items={fields.map((f) => f.id)} strategy={verticalListSortingStrategy}>
          {fields.map((field, index) => (
            <SortableField key={field.id} id={field.id} index={index} field={field} remove={remove} control={control} />
          ))}
        </SortableContext>
      </DndContext>

      {errors.fields && <p className="text-red-600">{errors.fields.message}</p>}
