...
const SideCategory: React.FC<{
	item: SideCategoryProps;
	activePath: string;
	child?: boolean;
}> = ({ item, activePath, child }) => {
			...
			<div className="flex items-center p-4 gap-2.5 select-none h-[46px]">
				{!hasChildren ? (
					<Link
						to={item.to}
						className={cn(
							`flex items-center gap-5 w-full h-[54px] `,
							isActive ? "font-bold" : "font-medium",
							child && "1xl:pl-8",
						)}
					>
						{item.icon}
						<span className="font-['Akatab'] text-black h-[22px]">
							{item.label}
						</span>
					</Link>
				) : (
					<>
						...
						>
							<span className={`font-['Akatab'] text-black 1xl:h-[22px]`}>
								{item.label}
							</span>
							<ChevronDown
								className={cn(
									"w-4 h-4 text-black transition-transform duration-600 p-[3px]",
									isOpen && "rotate-180",
								)}
							/>
						</button>
...
const Sidebar: React.FC<DashboardSidebarProps> = ({
	...
	const navigationItems: SideCategoryProps[] = [
		...
		},
		{
			...
			icon: (
				<Icons path="/sidebarIcons/online-booking.svg" alt="Bookings icon" />
			),
			label: "Bookings",
		},
		{
		...
		},
		{
			to: "",
			icon: (
				<Icons path="/sidebarIcons/content.svg" alt="Content Management icon" />
			),
		...
		},
		{
			...
			icon: (
				<Icons path="/sidebarIcons/group-chat.svg" alt="Staff Members icon" />
			),
			label: "Staff Members",
		},
		{
			...
			icon: (
				<Icons path="/sidebarIcons/question.svg" alt="Contact Requests icon" />
			),
		...
		},
		{
			...
			icon: (
				<Icons
					path="/sidebarIcons/global-partners.svg"
					alt="our_partners icon"
				/>
			),
			label: "Our Partners",
		},
	...
	const filteredNavigation = navigationItems
		.map((item) => {
			...
			if (item.children) {
				...
				if (
					hasDynamicAccess(item.to, userRole, userPermissions) ||
	...
	return (
		<nav
			...
			className="h-screen overflow-y-auto custom-scrollbar-style"
		>
